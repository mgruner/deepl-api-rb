...
RSpec.describe 'deepl help' do
  it "shows usage information" do
    stdout, stderr, status = Open3.capture3('deepl --help')
...
RSpec.describe 'deepl authentication' do
  it "works with a valid (external) API key" do
    stdout, stderr, status = Open3.capture3('deepl usage-information')
    expect(status.exitstatus).to eq 0
  end

  it "fails with a missing (external) API key" do
    stdout, stderr, status = Open3.capture3({ 'DEEPL_API_KEY' => nil }, 'deepl usage-information')
  ...
  it "fails with a wrong (external) API key" do
    stdout, stderr, status = Open3.capture3({ 'DEEPL_API_KEY' => 'wrong_key' }, 'deepl usage-information')
    expect(status.exitstatus).to eq 1
...
RSpec.describe 'deepl languages' do
  it "works" do
    stdout, stderr, status = Open3.capture3('deepl languages')
    expect(status.exitstatus).to eq 0
...
RSpec.describe 'deepl translate' do
  ...
  it "fails on missing language" do
    stdout, stderr, status = Open3.capture3('deepl translate')
    expect(status.exitstatus).to eq 1
    expect(stderr).to eq "No value provided for required options '--target-language'\n"
  end

  it "works with minimal arguments" do
    stdout, stderr, status = Open3.capture3('deepl translate --source-language EN --target-language DE',
  ...
  it "fails with wrong target language" do
    stdout, stderr, status = Open3.capture3('deepl translate --source-language EN --target-language FALSE',
                                            stdin_data: "Please go home.")
    expect(status.exitstatus).to eq 1
